...
function transformToLis(obj){
  return Object.entries(obj).map(elemente => (`<li>${elemente[0]}: ${elemente[1]}</li>`));}
...
const count = (target, input) => {
  ...
  return mergedArr.filter((item)=>item ===target ).length;
};
...
const totalSum = (input) => {
  ...
  return mergedArr.reduce((acc, value) => acc + value);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let filteredArr=input.map(arr=>arr.filter(elemente=>elemente %5 ===0 && typeof elemente === 'number'));
  return filteredArr.map(arr=>arr.map(item=>Math.pow(2,item)));
};
