...
const toLastNames = people => {
  ...
  let fullName = people.reduce((accumulator, names) => {
    accumulator.push(names.firstName + " " + names.lastName);
    return accumulator;
  }, []);
  return fullName;
};
...
const countNumberOfElements = (arr) => {
  ...
  let count = arr.reduce((accumulator) => ++accumulator, 0);
  return count;
};
...
const returnNames = (arr) => {
  let names = arr.reduce((accumulator, val) => {
...
const reversedString = (str) => {
  ...
  let newStr =str.split("").reduce((accumulator, char)=> char + accumulator, "");
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let numberOfChildren = arr.reduce((accumulator, obj) => {
    if(obj.children)
accumulator += obj.children.length;
    return accumulator;
  }, 0)
